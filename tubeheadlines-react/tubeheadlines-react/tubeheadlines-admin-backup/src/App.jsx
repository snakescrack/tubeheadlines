...
function App() {
  ...
  const convertToSEOTitle = (title) => {
    ...
    let seoTitle = title.replace(/[^\w\s]/gi, '')
                .replace(/\s*\|.*$/, '')
                .replace(/\s*-\s*YouTube$/, '')
                .replace(/\s*\(.*?\)/, '');

    // Convert to uppercase instead of title case
    seoTitle = seoTitle.toUpperCase();

    const attentionWords = ['BREAKING', 'WATCH', 'EXCLUSIVE', 'REVEALED'];
    if (!attentionWords.some(word => seoTitle.includes(word))) {
      seoTitle = 'BREAKING: ' + seoTitle;
    }

    // Limit to reasonable length (e.g., 60 characters)
            ...
            {videos
              ...
              .map(video => (
                <div key={video.id} className="video-item">
                  <h4>{video.customHeadline.toUpperCase()}</h4>
                  {video.scheduledAt && (
